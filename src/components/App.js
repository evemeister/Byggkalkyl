...
function AIOffert({ projects }) {
  ...
  async function generate() {
    ...
    try {
      const res  = await fetch("/api/generera-offert",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({jobDesc,jobType,kundNamn,timpris,matUpplag,rot,jobSize})});
      const data = await res.json();
      if(data.error) throw new Error(data.error);
      const mats = (data.material||[]).map((x,i)=>({id:i+1,...x}));
      setArbete(data.arbetsbeskrivning||""); setVillkor(data.villkor||"30 dagar netto.");
      setMaterial(mats);
